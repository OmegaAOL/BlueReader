 ...
 ******************************************************************************/
package org.omegaaol.bluereader.views.liststatus
...
import org.omegaaol.bluereader.activities.RedditTermsActivity
...
class ErrorView(activity: AppCompatActivity, error: RRError) : StatusListItemView(activity) {
	...
	init {
		...
		error.resolution?.apply {
			when (this) {
				RRError.Resolution.ACCEPT_BLUESKY_TERMS -> {
					resolveButton.setText(R.string.bsky_terms_error_resolution_button)
