...
import org.omegaaol.bluereader.common.GlobalExceptionHandler;
...
public abstract class BaseActivity extends AppCompatActivity
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		if(baseActivityIsToolbarActionBarEnabled()) {
			...
			if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
				// Insets to fix edge-to-edge ... required on SDK35+, breaks status bar color
				Window window = getWindow();

				WindowCompat.setDecorFitsSystemWindows(window, true);
				ViewCompat.setOnApplyWindowInsetsListener(outerView, (v, insets) -> {
				...
				PrefsUtility.applyStatNavCol(this, outerView);
			}
	...
	public final void onSharedPreferenceChanged(
		...
		if(key.startsWith(getString(R.string.pref_menus_appbar_prefix))
				...
				|| key.equals(getString(R.string.pref_pinned_feeds_key))) {
