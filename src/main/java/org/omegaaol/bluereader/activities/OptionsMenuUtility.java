...
public final class OptionsMenuUtility {
	...
	public static <E extends BaseActivity & OptionsMenuListener> void prepare(
		...
		add(
				...
				menu,
				Option.ACCENT,
				getOrThrow(appbarItemsPrefs, AppbarItemsPref.ACCENT),
				false);
	...
	private static void add(
		...
		switch(option) {
			...
			case THEMES: {
				final MenuItem themes = menu.add(
						...
						.setOnMenuItemClickListener(item -> {
							...
							dialog.setNegativeButton(R.string.dialog_close, null);
									...
									(dialog1, item1) -> {
										prefs.edit()
												.putString(
														...
														themeValues[item1])
												.apply();
										PrefsUtility.applyThemeAccent(activity);
										activity.recreate();
									});
			...
			case ACCENT: {
				final MenuItem accent = menu.add(
								Menu.NONE,
								AppbarItemsPref.ACCENT.ordinal(),
								Menu.NONE,
								R.string.options_accent)
						.setOnMenuItemClickListener(item -> {

							final SharedPrefsWrapper prefs
									= General.getSharedPrefs(activity);

							final PrefsUtility.AppearanceAccent currentAccent
									= PrefsUtility.appearance_accent();

							final String[] accentNames = activity.getResources()
									.getStringArray(R.array.pref_appearance_accent);

							final String[] accentValues = activity.getResources()
									.getStringArray(R.array.pref_appearance_accent_return);

							int selectedPos = -1;
							for(int i = 0; i < accentValues.length; i++) {
								if(PrefsUtility.AppearanceAccent.valueOf(
												StringUtils.asciiUppercase(accentValues[i]))
										.equals(currentAccent)) {
									selectedPos = i;
									break;
								}
							}
							final AlertDialog.Builder dialog
									= new AlertDialog.Builder(activity);
							dialog.setTitle(R.string.pref_appearance_accent_title);
				...
				accent.setShowAsAction(showAsAction);
				accent.setIcon(R.drawable.ic_themes_dark); //fix
