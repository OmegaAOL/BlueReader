...
import org.omegaaol.bluereader.common.AndroidCommon.runOnUiThread
import org.omegaaol.bluereader.common.PrefsUtility.AppearanceTwopane
...
object General {
                ...
                when (status) {
                    400, 401, 403, 404 -> {
                        ...
                        if (uri != null && uri.host != null) {
                            if ("bsky.social".equals(uri.host, ignoreCase = true)
                                || uri.host.endsWith(".bsky.social")
                            ) {
	...
	fun showMustReloginDialog(activity: AppCompatActivity) {
		AlertDialog.Builder(activity)
			.setTitle(R.string.bsky_relogin_error_title)
			.setMessage(R.string.bsky_relogin_error_message)
			.setPositiveButton(
