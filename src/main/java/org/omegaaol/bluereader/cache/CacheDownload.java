...
public final class CacheDownload extends PrioritisedCachedThreadPool.Task {
	...
	public CacheDownload(
		...
		mRequest = HTTPBackend.getBackend().prepareRequest(
				initiator.context,
				new HTTPBackend.RequestDetails(mInitiator.url, mInitiator.requestBody, false));
	}
	...
	public static int howManyCalls = 0;
	...
	private void performDownload(final HTTPBackend.Request request) {
		...
		if(mInitiator.queueType == CacheRequest.DOWNLOAD_QUEUE_REDDIT_API) {
			...
			RedditOAuth.AccessToken accessToken
					= mInitiator.user.getAccessToken();

			if(accessToken == null || accessToken.isExpired()) {

				mInitiator.notifyProgress(true, 0, 0);

				final RedditOAuth.FetchTokensResult result;

				if(mInitiator.user.isAnonymous()) {
					result = null; // TODO anonymous accounts
				} else {
					howManyCalls++;
					Log.e("OAuth", "Starting attempt number " + howManyCalls);
					result = RedditOAuth.fetchTokensWithRefresh(
							mInitiator.context,
							mInitiator.user.refreshToken);
					Log.e("OAuth", "Attempt finished");
				}

				if(result.status != RedditOAuth.FetchTokenResultStatus.SUCCESS) {
					mInitiator.notifyFailure(result.error);
					return;
				}

				mInitiator.user.setTokens(result.accessToken, result.refreshToken);
			}

			request.addHeader("Authorization", "Bearer " + mInitiator.user.getAccessToken());
		}
