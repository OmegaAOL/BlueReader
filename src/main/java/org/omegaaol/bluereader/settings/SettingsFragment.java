...
import org.omegaaol.bluereader.activities.ChangelogActivity;
...
import org.omegaaol.bluereader.common.FileUtils;
...
public final class SettingsFragment extends PreferenceFragmentCompat {
	...
	public void onDisplayPreferenceDialog(Preference preference) {
		if (preference instanceof ListPreference) {
			SysDialogFragment dialogFragment =
					SysDialogFragment.newInstance(preference.getKey());
	...
	public void onCreatePreferences(
		...
		if(versionPref != null) {
			versionPref.setSummary(
					BlueReader.getInstance(context).getPackageInfo().getVersionName());
		}
		...
		if(backupPreferencesPref != null) {
			backupPreferencesPref.setOnPreferenceClickListener(preference -> {
				...
				final Intent intent = new Intent(Intent.ACTION_CREATE_DOCUMENT)
						.setType("application/vnd.bluereader.prefsbackup")
		...
		if(Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
			...
			for(final int key : new int[] {
				...
				if(pref != null) {
					...
					pref.setSummary(R.string.pref_not_supported_before_kitkat);
				}
			}
		}
